 ...
 * textColor: rgba color array - defaults to opaque black
 * backgroundColor: rgba color array - defaults to transparent
 * borderColor: rgba color array - defaults to transparent
...
export const Text = ({
  ...
  const hasBackground = useMemo(() => backgroundColor[3] > 0, [backgroundColor])
  const hasBorders = useMemo(() => borderRadius > 0 || borderWidth > 0, [borderRadius, borderWidth])  
  ...
  }, [backgroundColor, borderColor, borderRadius, borderSmoothing, borderWidth, textColor, texture])
  ...
  const update = useCallback((self) => {
    ...
    const anchorOffset = {
      x: anchorHorz === LEFT ? -box.min.x : anchorHorz === CENTER ? -sphere.center.x : anchorHorz === RIGHT ? -box.max.x : 0,
      y: anchorVert === TOP ? box.min.y : anchorVert === CENTER ? sphere.center.y : anchorVert === BOTTOM ? box.max.y : 0
    }
    ...
    const position = [(anchorOffset.x * scale) + (placementOffset.x), (anchorOffset.y * scale) + (placementOffset.y), 0]

    self.scale.set(scale, scale, scale)
    self.position.set(...position)
    self.rotation.set(Math.PI, 0, 0)
  }, [anchorHorz, anchorVert, positionHorz, positionVert, scale, viewport.height, viewport.width])
